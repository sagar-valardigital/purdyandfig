...
(() => {
  if (window.SkioPlanPicker) {
    return Error("Skio: Skio already loaded");
  }
  ...
  window.SkioPlanPicker = ({ $planPicker, overrides = {} } = {}) => {
    try {
      ...
      assert(keyCount === 1, `Key needs to be unique, key: '${skio.key}' used ${keyCount} times`, {
        exit: true,
      });
      ...
      if (skio.formAttr) {
        skio.form = document.getElementById(skio.formAttr);
      }
      if (!skio.form) {
        skio.form = $planPicker.closest('form[action*="/cart/add"]');
      }
      assert(skio.form, "Couldn't find form, either connect to a form or submit using javascript", {
        messageType: "warn",
      });

      // PRODUCT
      if (overrides.product) {
        skio.product = overrides.product;
      } else {
      ...
      const getVariantId = () => {
        try {
          ...
          assert(
            false,
            "Failed to get variant id. No form and no getVariantId() override provided",
            { exit: true }
          );
        } catch (err) {
          console.error(err);
      ...
      skio.updateSellingPlanGroupAvailability = () => {
        try {
          ...
          const $$sellingPlanGroup = $planPicker.querySelectorAll(
            skio.selectors.sellingPlanGroup()
          );
          ...
          $$sellingPlanGroup.forEach(($sellingPlanGroup) => {
            ...
            if (isAvailable) {
              $groupContainer.classList.add("skio-group-container--available");
            } else {
              $groupContainer.classList.remove("skio-group-container--available");
            }
          ...
          if (checkedNoLongerAvailable) {
            ...
            if ($onetime && availableSellingPlanGroupIds.length === 0) {
              ...
              const $firstAvailableSellingPlanGroup = $planPicker.querySelector(
                `${skio.selectors.sellingPlanGroup()}:not(:disabled)`
              );
              if ($firstAvailableSellingPlanGroup) {
                $firstAvailableSellingPlanGroup.checked = true;
              }
            }
          }
      ...
      const getSelectedSellingPlanGroupId = () => {
        try {
          $planPicker
            ...
            .forEach((el) =>
              el
                ...
                .classList.remove("skio-group-container--selected")
            );
          const $onetime = $planPicker.querySelector(skio.selectors.onetime);
          if ($onetime?.checked) {
            $onetime
          ...
          if (
            assert($selectedSellingPlanGroup, "No selected selling plan group", {
              messageType: "warn",
            })
          ) {
            return null;
      ...
      const validateSellingPlanId = (sellingPlanId) => {
        try {
          ...
          const variant = getVariant();
          const index = variant.selling_plan_allocations.findIndex(
            (selling_plan_allocation) => selling_plan_allocation.selling_plan_id === sellingPlanId
          );
          const found = index !== -1;
      ...
      const updateUrlParam = (sellingPlanId) => {
        if (sellingPlanId !== "" && !validateSellingPlanId(sellingPlanId)) {
          sellingPlanId = "";
        }
      ...
      skio.updateSellingPlan = () => {
        try {
            ...
            if ($planPicker.querySelectorAll(skio.selectors.sellingPlans()).length > 1) {
              $planPicker.querySelectorAll(skio.selectors.sellingPlans()).forEach((selector) => {
                const option = Array.from(selector.options).find(
                  (option) => option.text == sellingPlanName
                );
                if (option) {
          ...
          updateDiscounts();
          updatePrices();
          $planPicker.dispatchEvent(
            new CustomEvent("skio:plan-picker:update", { detail: { skio } })
          );
        } catch (err) {
          console.error(err);
        }
      ...
      const updateDiscounts = () => {
        skio.availableSellingPlanGroupIds.forEach((availableSellingPlanGroupId) => {
          ...
          const $sellingPlans = $sellingPlanGroup
            .closest(skio.selectors.groupContainer)
            .querySelector(skio.selectors.sellingPlans());
          if (assert($sellingPlans, "No selling plans element found", { messageType: "info" })) {
            return;
          }
          const $$discount = $sellingPlanGroup.parentElement.querySelectorAll(
          ...
          const selling_plan_group = skio.product.selling_plan_groups.find(
            (selling_plan_group) => selling_plan_group.id === availableSellingPlanGroupId
          );
          const selling_plan = selling_plan_group.selling_plans.find(
            (selling_plan) => selling_plan.id === parseInt($sellingPlans.value)
          );
          ...
          switch (price_adjustment.value_type) {
            case "percentage":
              ...
              current_discount_absolute = Math.round(
                (skio.variant.price * price_adjustment.value) / 100.0
              );
              break;
            case "fixed_amount":
              current_discount_percent = Math.round(
                ((price_adjustment.value * 1.0) / skio.variant.price) * 100.0
              );
              current_discount_absolute = price_adjustment.value;
              break;
            case "price":
              current_discount_percent = Math.round(
                (((skio.variant.price - price_adjustment.value) * 1.0) / skio.variant.price) * 100.0
              );
          ...
          if (current_discount_percent == 0) {
            ...
            current_discount_text = skio.moneyFormatter.format(current_discount_absolute / 100);
          }
        ...
        if ($discountProperty) {
          const $onetime = $planPicker.querySelector(skio.selectors.onetime);
          const $selectedSellingPlanGroup = $planPicker.querySelector(
            skio.selectors.sellingPlanGroup(skio.selectedSellingPlanGroupId)
          );
          const $$discount = $selectedSellingPlanGroup.parentElement.querySelectorAll(
            skio.selectors.discount
          );
          if ($onetime?.checked || $$discount.length === 0) {
            ...
            if ($discount) {
              $discountProperty.value = $discount.innerHTML;
            }
          }
        }
      };
      ...
      const updatePrices = () => {
        ...
        skio.availableSellingPlanGroupIds.forEach((availableSellingPlanGroupId) => {
          ...
          const $subscriptionPrice = $sellingPlanGroup.parentElement.querySelector(
            skio.selectors.subscriptionPrice
          );
          if (assert($subscriptionPrice, "No price element found", { messageType: "info" })) {
            return;
          }
      ...
      $planPicker.querySelectorAll(skio.selectors.sellingPlans()).forEach((el) => {
        const handler = (e) => {
          const $sellingPlanGroup = e.currentTarget
            .closest(skio.selectors.groupContainer)
            .querySelector(skio.selectors.groupInput);
          if ($sellingPlanGroup.checked) {
            skio.updateSellingPlan();
          } else {
      ...
      skio.form.querySelector(skio.selectors.variantId).addEventListener("change", skio.update);
  ...
  window.SkioPlanPickerAutoInit = () => {
    const $$autoInitPlanPicker = document.querySelectorAll("[skio-plan-picker][skio-auto-init]");
